...
gulp.task('copy-goog', function(){
    gulp.src('./node_modules/google-closure-library/**/*')
        .pipe(gulp.dest(config.path.dist + '/js/closure-library/'));
});

gulp.task('clone-blockly', function () {
    shell.exec('git clone https://github.com/google/blockly.git dist/js/blockly/');
});

gulp.task('post-install', ['copy-goog', 'clone-blockly']);
